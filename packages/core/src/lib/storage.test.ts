...
describe("signUpload (local driver)", () => {
  it("returns a key, an absolute single-segment upload url, and a public url", async () => {
    const { key, uploadUrl, publicUrl } = await signUpload({ contentType: "image/png", ext: "png" });
    expect(key).toMatch(/^products\/[0-9a-f-]{36}\.png$/);
    // Hono's :key matches one segment, so the upload URL carries only the filename.
    const fileName = key.split("/").pop()!;
    expect(uploadUrl).toBe(`http://localhost:4000/api/admin/uploads/blob/${fileName}`);
