...
describe("withApiHandler", () => {
  it("runs the handler and attaches rate-limit headers", async () => {
  ...
  it("enforces its own rate limit with a Retry-After header", async () => {
    ...
    await redis.del("rl:/api/test/rl:14.14.14.14");
  });
});
...
describe("permission gating", () => {
  ...
  it("requirePermission throws 403 when the user lacks the permission", async () => {
    await withSession(noneId, async () => {
      await expect(requirePermission(Permission.ProductsWrite)).rejects.toMatchObject({ code: 403 });
    });
  });
