...
import { CATEGORIES, CONDITIONS, CATEGORY_LABEL, CONDITION_LABEL } from "@mogadget/contracts/constants";
...
export function CatalogFilters({ facets }: { facets: IFacets }) {
  ...
  const hasFilters =
    !!activeCategory || activeConditions.length > 0 || !!params.get("q") || !!params.get("min") || !!params.get("max");
          ...
          {CONDITIONS.map((cond) => (
            <label key={cond} style={checkRow}>
              ...
              <span>
                ...
                {facets.conditions[cond] ? <span style={muted}>{facets.conditions[cond]}</span> : null}
              </span>
            </label>
          ))}
        </div>
...
const chipOn: CSSProperties = { background: "var(--brand)", color: "#fff", borderColor: "var(--brand)" };
const checkRow: CSSProperties = { display: "flex", gap: 8, alignItems: "center", font: "400 15px var(--font-body)", cursor: "pointer" };
