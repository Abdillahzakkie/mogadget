...
export function AdminStats() {
  ...
  const live = products.filter((p) => p.isVisible && p.status !== "SOLD" && p.status !== "OUT_OF_STOCK");
    ...
    <div style={strip}>
      {cards.map((c) => (
        <div key={c.label} style={card}>
          <div className="price" style={{ fontSize: 24, fontWeight: 700, color: c.accent ?? "var(--ink)" }}>
            {c.value}
          </div>
