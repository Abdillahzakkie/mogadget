...
describe("adminApi CRUD", () => {
  ...
  it("uploadFile signs then PUTs the bytes to the returned upload url", async () => {
    post.mockResolvedValue(
      envelope({ uploadUrl: "http://api/blob/abc.jpg", key: "products/abc.jpg", publicUrl: "http://api/uploads/products/abc.jpg" }),
    );
    ...
    expect(post).toHaveBeenCalledWith("/admin/uploads/sign", { contentType: "image/jpeg", ext: "jpg" });
    expect(fetchMock).toHaveBeenCalledOnce();
    const [url, init] = fetchMock.mock.calls[0]!;
    expect(url).toBe("http://api/blob/abc.jpg");
    expect((init as { method: string }).method).toBe("PUT");
    expect(res).toEqual({ key: "products/abc.jpg", publicUrl: "http://api/uploads/products/abc.jpg" });
  });

  it("uploadFile defaults ext and content-type for a file lacking them", async () => {
    post.mockResolvedValue(envelope({ uploadUrl: "http://api/blob/y", key: "products/y.jpg", publicUrl: "u" }));
    ...
    expect(post).toHaveBeenCalledWith("/admin/uploads/sign", { contentType: "image/jpeg", ext: "noext" });
  });

  it("uploadFile throws when the storage PUT fails", async () => {
    post.mockResolvedValue(envelope({ uploadUrl: "http://api/blob/x", key: "products/x.jpg", publicUrl: "u" }));
