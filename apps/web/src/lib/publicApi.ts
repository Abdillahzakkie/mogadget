...
// Server-side reads go straight to the API origin (the /api rewrite is a browser-only
// convenience). Responses are tagged for on-demand ISR: an admin mutation pings the
// /revalidate webhook with these same tags, refreshing the affected pages within seconds.
// `revalidate: 300` is a time-based backstop. Tag strings MUST match core's revalidateTags.
