...
function watchConsole(page: Page): { errors: string[] } {
  ...
  page.on("requestfailed", (r) => {
    const u = r.url();
    // wa.me is deliberately aborted by the CTA test; ignore favicon.
    if (!u.startsWith("https://wa.me") && !u.includes("favicon")) {
...
test.describe("public catalog", () => {
  test("home renders featured products with images that actually load", async ({ page, request }) => {
  ...
  test("product detail integrates data, specs, and the WhatsApp deep link", async ({ page, request }) => {
    const c = watchConsole(page);
    const products = await apiProducts(request);
  ...
  test("tapping WhatsApp fires a click beacon that persists to the store", async ({ page, context, request }) => {
    ...
    const before = (await request.get(`${API}/api/products/${target.slug}`)).ok()
      ? ((await (await request.get(`${API}/api/products/${target.slug}`)).json()).data as { whatsappClickCount: number }).whatsappClickCount
      : 0;

    await page.goto(`/products/${target.slug}`);
    const clickReq = page.waitForRequest((r) => /\/api\/products\/.*\/click$/.test(r.url()));
    await page.getByRole("link", { name: /WhatsApp/ }).first().click({ noWaitAfter: true });
